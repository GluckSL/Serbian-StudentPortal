...
export class MatchingComponent implements OnInit, OnDestroy {
  ...
  checkAll() {
    ...
    const submissions = this.pairs.map(pair =>
      this.svc.submitAnswer(this.attempt._id, {
        questionId: pair.questionId,
        typedWord: pair.selectedRight || '',
      })
    );
    forkJoin(submissions).subscribe({
      next: (results) => {
        let correct = 0;
        results.forEach((r, i) => {
          this.pairs[i].isCorrect = r.isCorrect;
          if (r.isCorrect) correct++;
        });
        this.correctCount = correct;
        this.score = results.reduce((sum, r) => sum + (r.pointsEarned || 0), 0);
