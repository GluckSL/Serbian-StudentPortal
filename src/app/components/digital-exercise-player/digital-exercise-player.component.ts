...
export class DigitalExercisePlayerComponent implements OnInit, OnDestroy {
  ...
  startImagePinDrag(pq: PlayerQuestion, labelId: string, event: PointerEvent): void {
    ...
    // Defer preventDefault / pointer capture until a horizontal drag is confirmed
    // so vertical swipes scroll the page and taps on Next/Previous still work.
    this.clearImagePinInteractionState();
  ...
  onImagePinPointerUp(event: PointerEvent): void {
    ...
    if (!this.imagePinDrag.active) return;
    if (this.imagePinCapturePointerId != null && event.pointerId !== this.imagePinCapturePointerId) {
      return;
    }
  ...
  onImagePinPointerCancel(event: PointerEvent): void {
    ...
    if (this.imagePinDrag.active) {
      if (
        ...
        event.pointerId !== this.imagePinCapturePointerId
      ) {
        return;
      }
      this.resetImagePinDrag();
  ...
  /** Release any in-progress pin drag so navigation buttons stay tappable on mobile. */
