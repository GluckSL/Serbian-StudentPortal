  ...
  generateMissingAnswers(): void {
    ...
    this.reviewQuestions.forEach((q, i) => {
      ...
      if (Array.isArray((q as any).subQuestions)) {
        ((q as any).subQuestions as any[]).forEach((sq: any, si: number) => {
          if (sq.type === 'fill-blank') {
            ...
            const hasSample = Array.isArray(sq.sampleAnswers) && sq.sampleAnswers.some((a: string) => String(a || '').trim());
            if (hasSample) return;
          } else if (sq.type === 'jumble-word') {
            if (String(sq.expectedWord || '').trim()) return;
          } else {
            return;
          }
          const fi = this.reviewQuestions.length + sqLookup.size;
          sqLookup.set(fi, { parentIndex: i, subIndex: si });
          candidates.push({ index: fi, q: sq as ReviewQuestion });
        });
      }
    });
    ...
    this.exerciseService.generateMissingAnswers(payload).subscribe({
      next: (res) => {
        ...
        (res.results || []).forEach((r: any) => {
          ...
          if (sqm) {
            const parentQ = this.reviewQuestions[sqm.parentIndex] as any;
            if (!parentQ) return;
            ...
            if (sq.type === 'jumble-word' && r.expectedWord) {
          ...
          if (idx < 0 || idx >= this.reviewQuestions.length) return;
