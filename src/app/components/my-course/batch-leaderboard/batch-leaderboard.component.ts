...
export class BatchLeaderboardComponent implements OnInit, OnChanges {
  ...
  get activeMatesLabel(): string {
    const n = this.activeMatesCount;
    const mates = n === 1 ? 'član grupe' : 'članova grupe';
    if (this.subTab === 'today') return `${n} ${mates} uči danas`;
    if (this.subTab === 'weekly') return `${n} ${mates} uči ove nedelje`;
    return `${n} ${mates} uči ukupno`;
  }
  ...
  getActivityLabel(entry: LeaderboardEntry): string {
    ...
    if (entry.exercisesTotal > 0) {
      ...
      const n = entry.exercisesCompleted;
      parts.push(n === 1 ? '1 vežba završena' : `${n} vežbi završeno`);
    }

    if (parts.length) return parts.join(' · ');
    if (entry.loggedToday) return 'Prijavljen/a · još nema vremena učenja';
    return 'Tek počinje';
  }
  ...
  getRankUpHint(): string | null {
    ...
    if (this.myStats.engagementMinutes === above.engagementMinutes && abovePct > myPct) {
      return `Završite više vežbi da stignete do #${this.myRank - 1}`;
    }

    return `Nastavite da učite da stignete do #${this.myRank - 1}`;
  }
  ...
  getStatusBadge(entry: LeaderboardEntry): { label: string; cls: string } | null {
    ...
    if (this.subTab === 'today') {
      ...
      if (entry.rank === 3 && active) return { label: 'Rastuća zvezda', cls: 'badge--star' };
      if (avg >= 85 && entry.exercisesCompleted > 0) return { label: 'Bistar um', cls: 'badge--mind' };
      if (entry.exercisesCompleted > 0 && entry.dgSessionsCompleted > 0) return { label: 'Svestran', cls: 'badge--step' };
      if (entry.exercisesCompleted > 0) return { label: 'Entuzijasta', cls: 'badge--task' };
      if (entry.dgSessionsCompleted > 0) return { label: 'Govornik pro', cls: 'badge--task' };
      if (entry.arenaXp > 0) return { label: 'As Arene', cls: 'badge--arena' };
      if (entry.loggedToday) return { label: 'Na putu', cls: 'badge--neutral' };
