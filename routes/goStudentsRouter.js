...
router.post('/add', verifyToken, checkRole(['ADMIN', 'TEACHER_ADMIN']), async (req, res) => {
  try {
    ...
    const studentLang = Array.isArray(student.medium) && student.medium.includes(GO_LANGUAGE)
      ? GO_LANGUAGE
      : (student.medium || [])[0];
    if (studentLang && studentLang !== GO_LANGUAGE) {
      return res.status(400).json({
        message: `This student is on the ${studentLang} track. Add them from GO ${GO_LANGUAGE} management instead.`
      });
    }
