...
const { parsePdfBuffer, isAvailable: isPdfParseAvailable } = require('../services/pdfParseLoader');
...
async function extractPdfText(filePath) {
  ...
  try {
    ...
    return {
      text: result?.text || '',
      pages: Number.isFinite(Number(result?.numpages)) ? Number(result.numpages) : 1
    };
  } catch (err) {
